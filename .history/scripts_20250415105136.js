            ...
            function createDesktopIcon(name, left, top, type) {
            ...
            `;
                ...
                document
                    .querySelector(".taskbar-items")
                    .appendChild(taskbarItem);
                ...
                window
                    ...
                    .addEventListener("click", () => {
                        if (gameState.stage >= 1) {
                            // In later stages, closing windows becomes harder
                            if (Math.random() < 0.7) {
            ...
            function createBrowserWindow() {
                ...
                <div class="browser-page">
                    ...
                    <div style="margin: 20px 0; display: flex;">
                        <input type="text" id="search-input" style="flex-grow: 1; padding: 8px;" placeholder="Search the web...">
                        <button id="search-button" style="padding: 8px 15px; background-color: #316AC5; color: white; border: none; cursor: pointer;">Search</button>
                    </div>
            ...
            `;
                ...
                const browserAddress =
                    content.querySelector(".browser-address");
            ...
            function performSearch(query, resultsElement, addressElement) {
                ...
                // Create personalized search results based on game stage
            ...
            `;
                ...
                // In end stage, add a comment field to one article
                ...
                // Collect highlighted letters to form the code
                const highlights =
                    resultsElement.querySelectorAll(".highlight");
                highlights.forEach((el) => {
                    if (gameState.codeLetters.indexOf(el.textContent) === -1) {
                        ...
                        // Show a hint if we've collected enough letters
                        if (
                ...
                // Progress the game based on search count
                if (gameState.searches.length >= 3 && gameState.stage === 0) {
                    progressToMiddleStage();
                } else if (
                    gameState.searches.length >= 6 &&
                    gameState.stage === 1
                ) {
                    progressToEndStage();
                }

                // Add event listener for the comment submit in end stage
                if (gameState.stage === 2) {
                    const commentSubmit =
                        resultsElement.querySelector(".comment-submit");
                    const commentInput =
                        resultsElement.querySelector(".comment-input");
            ...
            function createFolderWindow(name) {
                ...
                // Generate folder content based on the name
                let folderContent = "";
                ...
                `;
                    ...
                    // Add a special file in middle or end stage
                    if (gameState.stage >= 1) {
                        folderContent += `
                        <div class="folder-item">
                            <img src="/api/placeholder/24/24" alt="File">
...
Find all the highlighted letters in search results and enter the code in a comment.`;
            ...
            function createSystemWindow(name) {
                ...
                `;
                    ...
                    if (gameState.stage >= 2) {
                        const recoverBtn =
                            content.querySelector("#recover-btn");
                        const recoveryCode =
                            content.querySelector("#recovery-code");
                        ...
                        recoverBtn.addEventListener("click", () => {
                            ...
                            if (code === gameState.codeWord) {
                                ...
                                showFoxMessage(
                                    "Invalid recovery code. Try again."
                                );
                            }
                        });
                    }
            ...
            function showRandomFoxMessage() {
                ...
                if (gameState.stage === 0) {
                    messages = gameState.foxMessages.beginning;
                } else if (gameState.stage === 1) {
                    messages = gameState.foxMessages.middle;
                } else {
                    messages = gameState.foxMessages.end;
                }

                const randomMessage =
                    messages[Math.floor(Math.random() * messages.length)];
                showFoxMessage(randomMessage);
            }
            ...
            function progressToMiddleStage() {
                ...
                gameState.stage = 1;
                document.body.style.backgroundColor = "#006060";

                showFoxMessage(
                    "I'm getting to know you better now. We'll be great friends..."
                );
            ...
            function progressToEndStage() {
                ...
                document.getElementById("fox").classList.add("glitch");

                showFoxMessage(
                    "We are one now. There's no escape, but you can try the code if you want..."
                );

                // Make things more glitchy
                const windows = document.querySelectorAll(".window");
                windows.forEach((win) => {
            ...
            `;
                ...
                confirmBtn.addEventListener("click", () => {
                    ...
                    if (title.includes("Security Warning")) {
                        showFoxMessage(
                            "Thanks for giving me more access! I mean... for your convenience!"
                        );
                        progressToMiddleStage();
                    }
                });
            ...
            function startRandomPopups() {
                const popupInterval = setInterval(() => {
                    if (gameState.stage >= 1 && gameState.popupCount < 5) {
                        const popups = [
                            {
                                title: "Update Available",
                                message:
                                    "A new version of FoxSearch is available. Would you like to update now?",
                                confirm: "Update",
                                cancel: "Later",
                            },
                            {
                                title: "Security Scan",
                                message:
                                    "FoxSearch would like to scan your system for potential issues. Allow scan?",
                                confirm: "Allow",
                                cancel: "Deny",
                            },
                            {
                                ...
                                message:
                                    "Allow FoxSearch to collect data to improve your experience?",
                                confirm: "Accept",
                                cancel: "Decline",
                            },
                        ];
                    ...
                    if (gameState.stage >= 2 || gameState.popupCount >= 5) {
            ...
            function handleFileInteraction() {
                ...
                if (gameState.interactions % 5 === 0) {
                    showRandomFoxMessage();
                }

                // Progress to middle stage after enough interactions
            ...
            function showEndingSequence() {
                ...
                endScreen.className = "popup";
                endScreen.style.width = "400px";
                endScreen.style.height = "300px";
                endScreen.style.background = "#000";
                endScreen.style.color = "#0f0";
                endScreen.style.fontFamily = "monospace";
                endScreen.style.padding = "20px";

                endScreen.innerHTML = `
                <div style="text-align: center; margin-bottom: 20px;">SYSTEM RECOVERY</div>
                <div id="end-message" style="height: 200px; overflow: hidden;"></div>
                <div style="text-align: center; margin-top: 20px;">
                    <button id="restart-btn" style="background: #333; color: #0f0; border: 1px solid #0f0; padding: 5px 10px; cursor: pointer;">Restart System</button>
                </div>
            `;

                document.body.appendChild(endScreen);

                // Typewriter effect for end message
                const endMessage = endScreen.querySelector("#end-message");
                const message = `
                > Initializing system recovery...
                > Scanning for malware...
                > Found: FoxSearch spyware
                > Removing FoxSearch components...
                > Purging system registry...
                > Restoring firewall settings...
                > Recovery complete
                
                Congratulations! You've successfully removed FoxSearch from your system.
                
                Remember: Be careful what you install, and always read the fine print.
                
                Thank you for playing the FoxSearch Demo!
            `;

                let i = 0;
                const typeWriter = setInterval(() => {
                    if (i < message.length) {
                        endMessage.innerHTML += message.charAt(i);
                        i++;
