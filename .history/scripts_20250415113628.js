...
function initFox() {
    ...
    setTimeout(() => {
        showFoxMessage("Click on me or the question mark button anytime you need a hint!");
    }, 6000);
...
function handleFileInteraction() {
    ...
    if (gameState.interactions === 3) {
        showFoxMessage("Good job interacting with files! Try searching for information in FoxSearch next.");
...
function progressToMiddleStage() {
    gameState.stage = 1;
    showFoxMessage("You think you're in control? Let's see... <span class='hint-text'>Pay attention to highlighted letters in search results!</span>");
...
function progressToEndStage() {
    gameState.stage = 2;
    showFoxMessage("We're one now. No turning back. <span class='hint-text'>Find all the highlighted letters to form the code word!</span>");
    
    desktop.classList.add("glitch");
    setTimeout(() => desktop.classList.remove("glitch"), 2000);
    
    setTimeout(() => {
        showFoxMessage("Check the Secret File or My Computer for clues about the complete code word.");
    }, 5000);
...
function createBrowserWindow() {
    ...
    if (gameState.searches.length === 0) {
        setTimeout(() => {
            showFoxMessage("Try searching for 'desktop' or 'organization' to find useful information!");
        }, 1500);
    }
...
function performSearch(query, resultsElement, addressElement) {
    ...
    highlights.forEach((el) => {
        if (gameState.codeLetters.indexOf(el.textContent) === -1) {
            ...
            if (gameState.codeLetters.length >= 2) {
                showFoxMessage(`You found a highlighted letter: <span class="hint-text">${el.textContent}</span>! You've collected ${gameState.codeLetters.length} letters so far: <span class="hint-text">${gameState.revealedCode}</span>`);
            }
        }
    });
        ...
        if (commentSubmit) {
            commentSubmit.addEventListener("click", () => {
                ...
                if (comment === gameState.codeWord) {
                    ...
                    const matchCount = countMatchingLetters(comment, gameState.codeWord);
                    showFoxMessage(
    ...
    if (gameState.searches.length === 1) {
        setTimeout(() => {
            showFoxMessage("Look carefully at the search results. Some letters might stand out!");
        }, 2000);
    }
}
...
function showEndingSequence() {
    ...
    setTimeout(() => {
        foxSpeech.innerHTML = "<span style='color:red;'>NO! YOU CAN'T DO THIS!</span>";
        foxSpeech.classList.remove("hidden");
